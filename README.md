...
- Поддерживает окно "крайних позиций без TP" через `closing.tp_no_tp_extreme_count_per_side`
  (для BUY — самые низкие входы, для SELL — самые высокие).
...
2. Проверьте настройки в `config.yaml`:
   ...
   - блок `trading` (символ, magic, интервалы, `duplicate_audit_interval_sec`),
